...
/// Reusable direct phone OTP form for `/auth` and guest Profile.
///
/// Preserves integration keys: `auth_phone_field`, `auth_phone_send`,
/// `auth_otp_field`, `auth_otp_verify`, `auth_phone_resend`.
class PhoneAuthPanel extends ConsumerStatefulWidget {
  const PhoneAuthPanel({super.key, this.clock});

  /// Optional clock for widget tests (fake time / cooldown).
  final DateTime Function()? clock;

  @override
  ConsumerState<PhoneAuthPanel> createState() => _PhoneAuthPanelState();
}

class _PhoneAuthPanelState extends ConsumerState<PhoneAuthPanel> {
      ...
      children: [
        ...
        if (_challengeId == null) ...[
          ...
          FilledButton(
            ...
            child: Text(_loading ? l10n.authPleaseWait : l10n.authSendCode),
          ),
        ] else ...[
          const SizedBox(height: AppTokens.spaceMd),
          ...
          FilledButton(
            ...
            child: Text(_loading ? l10n.authPleaseWait : l10n.authVerifyCode),
          ),
          TextButton(
            ...
            child: Text(
              canResend
                  ? l10n.authResendCode
                  : l10n.authResendInSeconds(_resendCooldown.remainingSeconds),
            ),
          ),
        ],
        if (_error != null) ...[
