import { config } from 'dotenv';
import { execFileSync } from 'child_process';
import { randomUUID } from 'crypto';
import { readFileSync } from 'fs';
import { resolve } from 'path';
import { PrismaClient } from '@prisma/client';
import {
  assertDedicatedTestDatabase,
  parseDatabaseName,
} from '../../common/security/origin.util';

// Prefer `.env.test`; never let a local `.env` (APP_ENVIRONMENT=dev) win.
config({ path: resolve(__dirname, '../../../.env'), quiet: true });
config({
  path: resolve(__dirname, '../../../.env.test'),
  override: true,
  quiet: true,
});
process.env.APP_ENVIRONMENT = 'test';

const backendRoot = resolve(__dirname, '../../..');
const migrationsDir = resolve(backendRoot, 'prisma/migrations');

const MIGRATIONS = [
  '20260722093227_init_auth',
  '20260722183000_verified_email_and_drop_fixture_inbox',
  '20260725120000_phone_only_auth',
] as const;

function requireTestDatabaseUrl(): string {
  assertDedicatedTestDatabase({
    APP_ENVIRONMENT: process.env.APP_ENVIRONMENT,
    DATABASE_URL: process.env.DATABASE_URL,
    TEST_DATABASE_NAME: process.env.TEST_DATABASE_NAME,
  });
  return process.env.DATABASE_URL as string;
}

function withDatabaseName(url: string, databaseName: string): string {
  const parsed = new URL(url);
  parsed.pathname = `/${databaseName}`;
  return parsed.toString();
}

function adminUrl(url: string): string {
  return withDatabaseName(url, 'postgres');
}

function prismaClient(url: string): PrismaClient {
  return new PrismaClient({
    datasources: { db: { url } },
  });
}

function runSql(url: string, sql: string): void {
  const prismaCli = require.resolve('prisma/build/index.js');
  try {
    execFileSync(
      process.execPath,
      [
        prismaCli,
        'db',
        'execute',
        '--stdin',
        '--schema',
        'prisma/schema.prisma',
      ],
      {
        cwd: backendRoot,
        env: { ...process.env, DATABASE_URL: url },
        input: sql,
        encoding: 'utf8',
        stdio: ['pipe', 'pipe', 'pipe'],
      },
    );
  } catch (error) {
    const err = error as {
      status?: number;
      stdout?: string;
      stderr?: string;
      message?: string;
    };
    const detail = [err.stderr, err.stdout, err.message]
      .filter(Boolean)
      .join('\n');
    throw new Error(detail || 'prisma db execute failed');
  }
}

function migrationSql(name: string): string {
  return readFileSync(resolve(migrationsDir, name, 'migration.sql'), 'utf8');
}

async function recreateDatabase(
  url: string,
  databaseName: string,
): Promise<void> {
  const admin = prismaClient(adminUrl(url));
  try {
    await admin.$executeRawUnsafe(
      `DROP DATABASE IF EXISTS "${databaseName}" WITH (FORCE)`,
    );
    await admin.$executeRawUnsafe(`CREATE DATABASE "${databaseName}"`);
  } finally {
    await admin.$disconnect();
  }
}

async function columnExists(
  client: PrismaClient,
  table: string,
  column: string,
): Promise<boolean> {
  const rows = await client.$queryRaw<Array<{ exists: boolean }>>`
    SELECT EXISTS (
      SELECT 1
      FROM information_schema.columns
      WHERE table_schema = 'public'
        AND table_name = ${table}
        AND column_name = ${column}
    ) AS "exists"
  `;
  return Boolean(rows[0]?.exists);
}

async function isPhoneNotNull(client: PrismaClient): Promise<boolean> {
  const rows = await client.$queryRaw<Array<{ is_nullable: string }>>`
    SELECT is_nullable
    FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'users'
      AND column_name = 'phone_e164'
  `;
  return rows[0]?.is_nullable === 'NO';
}

describe('phone_only_auth migration behavior', () => {
  jest.setTimeout(120_000);

  const sourceUrl = requireTestDatabaseUrl();
  const disposableName = `laforika_mig_wp03_${Date.now()}_test`;
  const disposableUrl = withDatabaseName(sourceUrl, disposableName);

  afterAll(async () => {
    const admin = prismaClient(adminUrl(sourceUrl));
    try {
      await admin.$executeRawUnsafe(
        `DROP DATABASE IF EXISTS "${disposableName}" WITH (FORCE)`,
      );
    } finally {
      await admin.$disconnect();
    }
  });

  it('migrates a clean database through the full history', async () => {
    await recreateDatabase(sourceUrl, disposableName);
    for (const name of MIGRATIONS) {
      runSql(disposableUrl, migrationSql(name));
    }

    const client = prismaClient(disposableUrl);
    try {
      expect(await columnExists(client, 'users', 'password_hash')).toBe(false);
      expect(
        await columnExists(client, 'auth_challenges', 'pending_password_hash'),
      ).toBe(false);
      expect(await columnExists(client, 'users', 'email_normalized')).toBe(
        true,
      );
      expect(await isPhoneNotNull(client)).toBe(true);
    } finally {
      await client.$disconnect();
    }
  });

  it('keeps a legacy phone-backed account and relations through phone-only migration', async () => {
    await recreateDatabase(sourceUrl, disposableName);
    runSql(disposableUrl, migrationSql(MIGRATIONS[0]));
    runSql(disposableUrl, migrationSql(MIGRATIONS[1]));

    const before = prismaClient(disposableUrl);
    const accountId = randomUUID();
    const sessionId = randomUUID();
    const familyId = randomUUID();
    const phone = `+98912${String(Date.now()).slice(-7)}`;
    const emailLocal = `legacy_${accountId.replace(/-/g, '').slice(0, 12)}`;
    const passwordHash = '$argon2id$placeholder';
    try {
      await before.$executeRawUnsafe(`
        INSERT INTO users (
          id, phone_e164, email_normalized, email_display, password_hash,
          email_verified_at, created_at, updated_at
        ) VALUES (
          '${accountId}'::uuid, '${phone}', '${emailLocal}@example.com',
          '${emailLocal}@example.com', '${passwordHash}',
          NOW(), NOW(), NOW()
        )
      `);
      await before.$executeRawUnsafe(`
        INSERT INTO auth_sessions (
          id, user_id, family_id, absolute_expires_at, last_seen_at, created_at
        ) VALUES (
          '${sessionId}'::uuid, '${accountId}'::uuid, '${familyId}'::uuid,
          NOW() + interval '30 days', NOW(), NOW()
        )
      `);
    } finally {
      await before.$disconnect();
    }

    runSql(disposableUrl, migrationSql(MIGRATIONS[2]));

    const after = prismaClient(disposableUrl);
    try {
      const users = await after.$queryRaw<
        Array<{ id: string; phone_e164: string }>
      >`SELECT id::text AS id, phone_e164 FROM users WHERE id = ${accountId}::uuid`;
      expect(users).toHaveLength(1);
      expect(users[0].id).toBe(accountId);
      expect(users[0].phone_e164).toBe(phone);

      const sessions = await after.$queryRaw<Array<{ id: string }>>`
        SELECT id::text AS id FROM auth_sessions WHERE id = ${sessionId}::uuid
      `;
      expect(sessions).toHaveLength(1);
      expect(await columnExists(after, 'users', 'password_hash')).toBe(false);
      expect(await isPhoneNotNull(after)).toBe(true);
    } finally {
      await after.$disconnect();
    }
  });

  it('fails safely when an account lacks phone identity before dropping password data', async () => {
    await recreateDatabase(sourceUrl, disposableName);
    runSql(disposableUrl, migrationSql(MIGRATIONS[0]));
    runSql(disposableUrl, migrationSql(MIGRATIONS[1]));

    const before = prismaClient(disposableUrl);
    const orphanId = randomUUID();
    const emailLocal = `orphan_${orphanId.replace(/-/g, '').slice(0, 12)}`;
    const passwordHash = '$argon2id$placeholder';
    try {
      await before.$executeRawUnsafe(`
        INSERT INTO users (
          id, phone_e164, email_normalized, email_display, password_hash,
          email_verified_at, created_at, updated_at
        ) VALUES (
          '${orphanId}'::uuid, NULL, '${emailLocal}@example.com',
          '${emailLocal}@example.com', '${passwordHash}',
          NOW(), NOW(), NOW()
        )
      `);
      expect(await columnExists(before, 'users', 'password_hash')).toBe(true);
    } finally {
      await before.$disconnect();
    }

    expect(() => runSql(disposableUrl, migrationSql(MIGRATIONS[2]))).toThrow(
      /phone-only migration blocked|lack phone identity/i,
    );

    const after = prismaClient(disposableUrl);
    try {
      expect(await columnExists(after, 'users', 'password_hash')).toBe(true);
      expect(await isPhoneNotNull(after)).toBe(false);
    } finally {
      await after.$disconnect();
    }
  });

  it('keeps historical migration SQL present and non-empty', () => {
    expect(parseDatabaseName(sourceUrl)).toMatch(/_test$/);
    for (const name of MIGRATIONS) {
      expect(migrationSql(name).length).toBeGreaterThan(0);
    }
  });
});
